...
references_regex = regex.compile('(.*)\ (REFERENCES)\ (.*)', regex.IGNORECASE)
...
#novalidate_regex = regex.compile('(.*)\ (ENABLE\s+(?: |NOVALIDATE))(.*)', regex.IGNORECASE)
...
def make_snow(sqlin, sqlout, no_comments):
    ...
    for line in sqlin:
        ...
        result = references_regex.match(sql)
        if result:
            pre = result.group(1)
            clause = result.group(2)
            post = result.group(3)
